...
def main():
    ...
    output, save_prefix = set_output(args, "eval_transmembrane_log", test=True)
    ...
    ## load a test dataset
    start = Print(" ".join(['start loading test datasets', data_cfg.path["test"]]), output)
    dataset_test = transmembrane.load_transmembrane(data_cfg, "test", alphabet, args["sanity_check"])
